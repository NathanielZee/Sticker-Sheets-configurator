...
function priceSheets(sheetSize: string, qty: number) {
  const q = Math.max(1, Math.floor(qty || 0))

  // Find the correct size table
  const table = PRICING.priceTable[sheetSize as keyof typeof PRICING.priceTable]
  ...
  // Find the largest quantity tier <= qty
  ...
  if (chosen) {
    total = table[String(chosen) as keyof typeof table]
    if (q < chosen) {
      unit = total / chosen
      total = unit * q
    } else {
  ...
  // Calculate savings based on baseline quantity (10) for this size
  const baselineTotal = table["10"]
  const baselineUnit = baselineTotal / 10
  const savePct = Math.round((1 - unit / baselineUnit) * 100)

  // Calculate next tier info
  const next = nextTierInfo(q)
  let nextTierData = null
  if (next && table[String(next.nextQty) as keyof typeof table] != null) {
    const nextUnit = Number(table[String(next.nextQty) as keyof typeof table]) / next.nextQty
    const nextSave = Math.round((1 - nextUnit / baselineUnit) * 100)
...
const sizes = [
  { label: "100 × 150 mm", width: 100, height: 150, key: "100x150" },
  { label: "213 × 275 mm", width: 213, height: 275, key: "213x275" },
]
...
export default function StickerCalculator() {
  ...
  if (quantity > 0) {
    try {
      const sheetSize = selectedSize?.key || (customWidth && customHeight ? "100x150" : "100x150")
      pricingResult = priceSheets(sheetSize, quantity)
      total = pricingResult.totalPrice
      unitPrice = pricingResult.unitPrice

      if (pricingResult.nextTier) {
        upsellMsg = `Save ${pricingResult.nextTier.nextSavePct}% when you add ${pricingResult.nextTier.addMore} sheets`
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  {PRICING.meta.qtyTiers.map((qty) => {
                    const currentSheetSize = selectedSize?.key || "100x150"
                    const table = PRICING.priceTable[currentSheetSize as keyof typeof PRICING.priceTable]
                    const price = table ? table[String(qty) as keyof typeof table] : 0
                    return (
          ...
          <div className="border-t border-gray-200 pt-4 sm:pt-6">
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="text-gray-600 text-xs sm:text-sm">
                ${quantity > 0 ? (finalTotal / quantity).toFixed(2) : "0.00"} / sheet
              </div>
            </div>
